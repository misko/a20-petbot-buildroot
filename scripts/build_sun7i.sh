...
build_buildroot()
{
    ...
    printf "\nMISKO  BR X1 ${LICHEE_BR_OUT}\n"
    sleep 1
    if [ ! -f ${LICHEE_BR_OUT}/.config ] ; then
        printf "\nXXXXXXXXXX Using default config ... ${LICHEE_BR_DEFCONF}\n\n"
        make O=${LICHEE_BR_OUT} -C ${LICHEE_BR_DIR} ${LICHEE_BR_DEFCONF}
    fi
    printf "\nMISKO  BR X2\n"
    sleep 1
    make O=${LICHEE_BR_OUT} -C ${LICHEE_BR_DIR} LICHEE_GEN_ROOTFS=n V=1
    printf "\nMISKO  BR X3\n"
    sleep 5
}
