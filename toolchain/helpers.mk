...
copy_toolchain_lib_root = \
	ARCH_SYSROOT_DIR_TMP="$(strip $1)"; \
	LIB="$(strip $2)"; \
	DESTDIR="$(strip $3)" ; \
 \
	LIBS=`(cd $${ARCH_SYSROOT_DIR_TMP}; \
		find -L "." -name "$${LIB}.*" )` ;\
	echo "CONSIDERING FOLDER $${ARCH_SYSROOT_DIR_TMP} LIB $${LIB} PF $${TOOLCHAIN_EXTERNAL_PREFIX} "; \
	for FILE in $${LIBS} ; do \
		LIB=`basename $${FILE}`; \
		LIBDIR=`dirname $${FILE}` ; \
		echo "CONSIDER $${FILE} FROM $${LIB}"; \
		echo "$${FILE} $${LIB} $${ARCH_SYSROOT_DIR_TMP} $${LIBDIR} $${LIB} WTFWTY "  ; \
		while test \! -z "$${LIB}"; do \
			FULLPATH="$${ARCH_SYSROOT_DIR_TMP}/$${LIBDIR}/$${LIB}" ; \
			echo rm -fr $(TARGET_DIR)/$${DESTDIR}/$${LIB}; \
			mkdir -p $(TARGET_DIR)/$${LIBDIR}; \
			if test -h $${FULLPATH} ; then \
				cp -d $${FULLPATH} $(TARGET_DIR)/$${FILE}; \
				echo "COPYING $${FULLPATH} $(TARGET_DIR)/$${DESTDIR}"; \
			elif test -f $${FULLPATH}; then \
				$(INSTALL) -D -m0755 $${FULLPATH} $(TARGET_DIR)/$${LIBDIR}/$${LIB}; \
				echo "INSTALL $${FULLPATH} $(TARGET_DIR)/$${DESTDIR}/$${LIB}"; \
			else \
				exit -1; \
			fi; \
 ...
 \
	echo -n
$(info $$copy_toolchain_lib_root is [${copy_toolchain_lib_root}])
#
